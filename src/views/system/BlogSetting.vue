  ...
  <div>
    ...
    <div style="width: 100%;">
			<span style="margin-top: 20px">
				状态:
				<el-radio-group @change="stateChange($event)" v-model="status" size="medium">
      ...
      <span>
        		标签：
				<el-select v-model="labelId" @change="labelChange($event)" clearable filterable placeholder="请选择">
					<el-option
    ...
    <div>
      <el-table :data="tableData" border stripe style="width: 100%">
        ...
        <el-table-column label="标签" align="center" width="100">
          <template slot-scope="scope">
            <div slot="reference" class="name-wrapper">
              <span v-for="item in scope.row.labels " style="margin-left: 10px">
                {{ item.name}}
              </span>
            </div>
          </template>
...
export default {
  ...
  methods: {
    stateChange(value) {
      this.initialize(this.labelId, value, this.title, 1, this.pageSize)
    },
    labelChange(value) {
      this.initialize(value, this.status, this.title, 1, this.pageSize)
    },
    deleteBlog(id) {
